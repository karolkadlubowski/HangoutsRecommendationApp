...
using MediatR;
using VenueList.API.Application.Handlers.DeleteFavoritesFromVenue;

namespace VenueList.API.Application.Handlers.Strategies
{
    public class VenueDeletedEventHandlerStrategy : BaseEventHandlerStrategy
    {
        ...
        public override async Task<DistributedTransactionResult> HandleEventAsync(Event @event, CancellationToken cancellationToken = default)
        {
            ...
            await _mediator.Send(new DeleteFavoritesFromVenueCommand(dataModel.VenueId));
